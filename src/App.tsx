...
export default function App() {
  ...
  const [ceritaTab, setCeritaTab] = useState<'pendaki' | 'leader'>('pendaki');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      ...
      console.error("App reviews fetch error:", error);
    });
    return () => unsubscribe();
  }, []);
        ...
        {showSplash && (
          ...
          >
            ...
            <motion.h1 
              ...
              className="text-4xl md:text-5xl font-black text-art-text mb-4"
            >
              <span className="uppercase tracking-tight">{config?.homepage?.splashTitleLine1 || "Ngopi Di"}</span>{" "}
              <span className="text-art-orange font-bold uppercase tracking-tight">{config?.homepage?.splashTitleLine2 || "Ketinggian"}</span>
            </motion.h1>
            <motion.p 
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              {config?.homepage?.splashDesc || "Open trip eksklusif yang menyediakan pengalaman mendaki dengan fasilitas lengkap dan secangkir kopi terbaik di atas awan."}
            </motion.p>
            <motion.div
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              <Button 
                ...
                disabled={loading}
              >
                {loading ? "Memuat Data..." : (config?.homepage?.splashButtonText || "Mulai Melihat Tour")} {!loading && <ChevronRight className="ml-2 group-hover:translate-x-1 transition-transform" />}
              </Button>
              
              <div className="grid grid-cols-2 gap-3">
                {config?.homepage?.showIgButton !== false && (
                  <Button variant="secondary" className="w-full text-[10px] tracking-widest uppercase font-bold" onClick={() => window.open(config?.homepage?.splashIgUrl || 'https://www.instagram.com/ngopi.dketinggian?igsh=Y3JtN3Y2eXIya29y', '_blank')}>
                    Kunjungi IG
                  </Button>
                )}
                {config?.homepage?.showTiktokButton !== false && (
                  <Button variant="secondary" className="w-full text-[10px] tracking-widest uppercase font-bold" onClick={() => window.open(config?.homepage?.splashTiktokUrl || 'https://tiktok.com/@ngopidiketinggian', '_blank')}>
                    Kunjungi TikTok
                  </Button>
                )}
              </div>
              
              {(!user && config?.homepage?.showGoogleLogin !== false) ? (
